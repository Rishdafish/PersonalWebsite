/*
  # Fix Registration and Token System
  ...
  2. Token System
    - Create valid tokens for specialized access
    - Ensure token validation works properly

  3. User Registration Flow
    - Handle new user creation with proper role assignment
    - Support token-based role upgrades
...
CREATE POLICY "Authenticated users can read active tokens"
  ON user_tokens FOR SELECT TO authenticated USING (is_active = true);

CREATE POLICY "Service role can manage tokens"
  ON user_tokens FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "All users can read published blog posts"
  ON blog_posts FOR SELECT TO authenticated USING (published = true);
...
CREATE POLICY "Allow authenticated users to create blog posts"
  ON blog_posts FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Allow users to manage own blog posts"
  ON blog_posts FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Allow authenticated users to create comments"
  ON blog_comments FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Allow users to manage own comments"
  ON blog_comments FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Admins can delete any comment"
  ON blog_comments FOR DELETE TO authenticated 
  USING (EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = auth.uid() AND user_profiles.role = 'admin'));
...
CREATE POLICY "Allow users to manage own subjects"
  ON subjects FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Allow users to manage own work entries"
  ON work_entries FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Service role can manage work entries"
  ON work_entries FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Achievements Policies
CREATE POLICY "Allow users to manage own achievements"
  ON achievements FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Allow users to manage own statistics"
  ON user_statistics FOR ALL TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Service role can manage statistics"
  ON user_statistics FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Insert valid tokens for specialized access
INSERT INTO user_tokens (token, description) 
VALUES 
  ('hello_1210', 'Primary specialized access token'),
