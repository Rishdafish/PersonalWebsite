/*
  ...
  1. Clear and recreate tokens table
  2. Insert valid tokens including hello_1210
...
BEGIN
    ...
    FOR token_count IN (SELECT token FROM user_tokens WHERE is_active = true) LOOP
        RAISE LOG 'Active token: %', token_count;
    END LOOP;
END $$;
