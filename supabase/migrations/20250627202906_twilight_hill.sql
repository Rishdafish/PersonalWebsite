/*
  ...
  1. Functions
    - Drop and recreate all trigger functions with proper error handling
    - Create manual profile creation function for frontend fallback
    - Add helper functions for achievements

  2. Triggers
    - Drop and recreate all triggers to avoid conflicts
    - Main trigger on auth.users for user registration
  ...
  4. Error Handling
    - All trigger functions have exception handling
    - Functions won't fail user creation if secondary operations fail
*/
...
DROP FUNCTION IF EXISTS public.create_default_achievements();
DROP FUNCTION IF EXISTS public.create_default_achievements_for_user(uuid);
