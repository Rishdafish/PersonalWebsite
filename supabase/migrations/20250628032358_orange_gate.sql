...
DO $$
BEGIN
  -- Drop policies on blog_posts that reference user_profiles.role
  DROP POLICY IF EXISTS "Admins can manage all blog posts" ON blog_posts;
  
  -- Drop policies on projects that reference user_profiles.role
  DROP POLICY IF EXISTS "Admins can manage all projects" ON projects;
  
  -- Drop policies on blog_comments that reference user_profiles.role
...
-- Recreate the policies that were dropped, now with proper enum casting
DO $$
BEGIN
  -- Recreate blog_posts admin policy
  ...
  -- Recreate blog_comments admin policy
