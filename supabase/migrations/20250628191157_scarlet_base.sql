/*
  # Complete User Role System Setup

  1. Database Schema
    - Create user_role enum type
    - Update user_profiles table to use enum
...
BEGIN
    IF NOT EXISTS (
        ...
        EXECUTE 'CREATE POLICY "Allow profile creation during signup"
            ON user_profiles
            FOR INSERT
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Allow user registration during signup'
    ) THEN
        EXECUTE 'CREATE POLICY "Allow user registration during signup"
            ON users
            FOR INSERT
            TO anon, authenticated, supabase_auth_admin
            WITH CHECK (true)';
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Allow achievement creation during signup'
    ) THEN
        EXECUTE 'CREATE POLICY "Allow achievement creation during signup"
            ON achievements
            FOR INSERT
            TO anon, authenticated, supabase_auth_admin
            WITH CHECK (true)';
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Allow statistics creation during signup'
    ) THEN
        EXECUTE 'CREATE POLICY "Allow statistics creation during signup"
            ON user_statistics
            FOR INSERT
            TO anon, authenticated, supabase_auth_admin
            WITH CHECK (true)';
