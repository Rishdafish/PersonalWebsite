/*
  # Fix User Authentication and Profile System
  ...
  2. Security
    ...
    - Create appropriate policies for signup process
...
-- Step 2: Drop existing function explicitly to avoid return type conflicts
DROP FUNCTION IF EXISTS create_user_profile_manual(uuid, text, text) CASCADE;
DROP FUNCTION IF EXISTS create_user_profile_manual(p_user_id uuid, p_user_email text, p_user_token text) CASCADE;
...
-- Step 4: Create manual profile creation function with explicit DROP first
