/*
  # Fix user_role enum and policies

  1. Database Changes
    - Ensure user_role enum exists in public schema
  ...
  2. Security
    ...
    - Add proper enum casting for admin policies
...
-- Drop existing admin policies that use enum casting
...
ALTER COLUMN role TYPE user_role USING 
  CASE 
    ...
    WHEN role = 'specialized' THEN 'specialized'::user_role
...
BEGIN
  ...
  ELSIF user_token IS NOT NULL AND EXISTS (
    SELECT 1 FROM user_tokens WHERE token = user_token AND is_active = true
  ) THEN
...
-- Recreate admin policies with proper enum casting
