/*
  ...
  1. Database Changes
    - Clear and repopulate user_tokens table with test tokens
    - Ensure hello_1210 token exists and is active
    - Add proper verification and debugging
  ...
  3. Verification
    - Test token existence with multiple query patterns
...
BEGIN
    ...
    -- Log all tokens for verification (using proper variable name)
    ...
    -- Additional verification with detailed token info
    FOR current_token IN (
        SELECT 'Token: ' || token || ', Active: ' || is_active::text || ', Created: ' || created_at::text
        FROM user_tokens 
        ORDER BY token
    ) LOOP
        RAISE NOTICE 'Token details: %', current_token;
    END LOOP;
END $$;
...
SELECT 'Direct query test' as test_type, * FROM user_tokens 
WHERE token = 'hello_1210' AND is_active = true;

-- Test the EXISTS pattern used in validation
SELECT 'EXISTS query test' as test_type, EXISTS(
    SELECT 1 FROM user_tokens 
    WHERE token = 'hello_1210' AND is_active = true
) as token_validation_result;

-- Test case-sensitive matching
SELECT 'Case sensitivity test' as test_type, 
       token, 
       length(token) as token_length,
       is_active
FROM user_tokens 
WHERE token LIKE '%hello%' OR token LIKE '%HELLO%';

-- Test with exact character matching
SELECT 'Character matching test' as test_type,
       token,
       token = 'hello_1210' as exact_match,
       token LIKE 'hello_1210' as like_match,
       is_active
FROM user_tokens;