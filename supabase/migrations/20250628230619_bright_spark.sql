/*
  # Final fix for user_role enum type issue

  This migration comprehensively addresses the "type user_role does not exist" error by:
  1. Ensuring the enum type exists and is properly defined
  2. Fixing all table columns that reference the enum
  3. Recreating all functions and triggers with proper error handling
  4. Setting up correct RLS policies for signup process
  5. Granting all necessary permissions

  ## Changes Made
  1. Drop and recreate user_role enum type safely
  2. Fix user_profiles table role column
  3. Recreate signup trigger function with comprehensive error handling
  4. Recreate manual profile creation function
